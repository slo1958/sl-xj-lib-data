...
Protected Class clDataTable
	#tag Method, Flags = &h0
		Function add_column(the_column as clAbstractDataSerie) As clAbstractDataSerie
		  ...
		  If Self.get_column(tmp_column_name) <> Nil Then
		    Raise New lib_data.clDataException("Column already exists " + tmp_column_name)
		    Return Nil
		  ...
		  Raise New lib_data.clDataException("Column cannot be added " + tmp_column_name)
	...
	#tag Method, Flags = &h0
		Sub append_row(the_row as clDataRow, create_columns_flag as boolean=True)
		  ...
		  For Each column As String In the_row
		    ...
		    If tmp_column = Nil Then
		      Raise New lib_data.clDataException("Adding row with unexpected column " + column)
	...
	#tag Method, Flags = &h0
		Function clone(preserve_links as Boolean = False) As clDataTable
		  ...
		  ' 
		  dim output_table as New lib_data.clDataTable(self.name+" copy")
		...
		Sub Constructor(the_table_name as string, the_columns() as clAbstractDataSerie, auto_clone_columns as boolean = false)
		  ...
		  For i As Integer = 0 To tmp_columns.Ubound
		    If tmp_columns(i) = Nil Then
		      Raise New lib_data.clDataException("Internal error")
		    ...
		    Elseif tmp_columns(i).is_linked_to_table And Not auto_clone_columns Then
		      Raise New lib_data.clDataException("Cannot add a linked serie to a new table, use select_columns() method instead.")
	...
	#tag Method, Flags = &h0
		Sub Constructor(the_table_name as string, the_source as RecordSet)
		  Dim tmp_table_name As String
		  dim record_source as RecordSet = the_source
		  
		  tmp_table_name = the_table_name
		  
		  internal_new_table(tmp_table_name)
		  
		  dim tmp_cols as new Dictionary
		  
		  for i as integer = 1 to record_source.FieldCount
		    dim tmp_col_name as string = the_source.IdxField(i).Name
		    dim tmp_serie_name as string = tmp_col_name
		    
		    tmp_cols.Value(tmp_col_name) = new clDataSerie(tmp_serie_name)
		    
		  next
		  
		  while not record_source.eof
		    for i as integer = 1 to record_source.FieldCount
		      dim tmp_col_name as string = the_source.IdxField(i).Name
		      dim tmp_serie as clDataSerie = clDataSerie(tmp_cols.Lookup(tmp_col_name, nil))
		      tmp_serie.append_element(record_source.IdxField(i).StringValue)
		      
		    next
		    
		    record_source.MoveNext
		  wend
		  
		  for each tmp_var as Variant in tmp_cols.Values()
		    dim tmp_serie as clDataSerie = clDataSerie(tmp_var)
		    call self.add_column(tmp_serie)
	...
	#tag Method, Flags = &h0
		Function groupby(grouping_dimensions() as string, aggregate_measures() as string, aggregate_mode() as string) As clDataTable
		  ...
		  Return New lib_data.clDataTable("results", output_series)
	...
	#tag Method, Flags = &h21
		Private Sub internal_add_row(the_row_data() as variant)
		  ...
		  If the_row_data.Ubound <> columns.Ubound Then
		    Raise new lib_data.clDataException("Invalid row in internal_add_row")
	...
	#tag Method, Flags = &h0
		Sub save_as_text(the_destination as FolderItem,the_line_parser as itf_row_parser, with_header  as Boolean)
	...
	#tag Method, Flags = &h0
		Function select_columns(column_names() as string) As clDataTable
		  ...
		  '
		  '
		  Dim res As New lib_data.clDataTable("select " + Self.table_name)
