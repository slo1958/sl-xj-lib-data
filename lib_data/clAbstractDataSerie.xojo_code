...
Implements lib_data.itf_json_able
		...
		Sub debug_dump()
		  ...
		  For row As Integer = 0 To row_count-1
		    ...
		    If element IsA clDataSerie Then
		      tmp_item.Append(lib_data.itf_json_able(element).to_json.ToString)
		      ok_convert = True
		    ...
		    If Not ok_convert Then
		      If element IsA lib_data.itf_json_able Then
		        tmp_item.Append(lib_data.itf_json_able(element).to_json.ToString)
		        ok_convert = True
		      End If
	...
	#tag Method, Flags = &h0
		Sub link_to_table(the_table as clDataTable)
		  ...
		  Else
		    Raise New lib_data.clDataException("Cannot redefine link to table for a serie")
	...
	#tag Method, Flags = &h0
		Function to_json() As JSONItem
		  ...
		  For row As Integer = 0 To row_count-1
		    ...
		    If element IsA lib_data.itf_json_able Then
		      js_list.append(lib_data.itf_json_able(element).to_json)
