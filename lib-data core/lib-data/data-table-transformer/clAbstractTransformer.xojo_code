...
Protected Class clAbstractTransformer
	...
	#tag Method, Flags = &h0
		Sub SetLabel(newLabel as string)
		  if self.StepLabel.trim.Length = 0 then
		    self.StepLabel = newLabel
		    
		  else
		    self.getLogManager.WriteError(CurrentMethodName," Cannot update label [%0] to [%1]", self.StepLabel, newLabel)
	...
	#tag Method, Flags = &h0
		Sub TraceInputToLog(StepLabel as string)
		  for each c as clTransformerConnection in self.InputConnectors.Values
		    
		    if c.GetTable = nil then
		      self.getLogManager.WriteInfo(CurrentMethodName _
		      , "Input connector [%0] of step [%1] without table" _ 
		      ...
		      self.getLogManager.WriteInfo(CurrentMethodName _
		      , "Input connector [%0] of step [%3] has table [%1] %2 records." _
		      , c.GetConnectorLabel _
		      , c.GetTable.Name _
	...
	#tag Method, Flags = &h0
		Sub TraceOutputToLog(StepLabel as string)
		  for each c as clTransformerConnection in self.OutputConnectors.Values
		    
		    if c.GetTable = nil then
		      self.getLogManager.WriteInfo(CurrentMethodName _
		      , "Output connector [%0] of step [%1] without table" _ 
		      ,  c.GetConnectorLabel _
		      , stepLabel _
		      )
		      ...
		      self.getLogManager.WriteInfo(CurrentMethodName _
		      , "Output connector [%0] of step [%3] has table [%1] %2 records." _
		      , c.GetConnectorLabel _
		      , c.GetTable.Name _
		      , c.GetTable.RowCount.ToString _
		      , stepLabel _
	...
	#tag Method, Flags = &h0
		Sub WriteLogkk(message as string, paramarray txt as string)
